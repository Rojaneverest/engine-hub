...
function initRenderer(){
  ...
  scene.add(new THREE.HemisphereLight(0xdfe8f2,0x2a2622,.55));
  const key=new THREE.DirectionalLight(0xffffff,1.5); key.position.set(6,10,-7); scene.add(key);
  const fill=new THREE.DirectionalLight(0xbcd4ff,.45); fill.position.set(-8,3,5); scene.add(fill);
  const rim=new THREE.DirectionalLight(0xffe6c8,.6); rim.position.set(0,5,10); scene.add(rim);
  try{ // soft-box studio environment for metal reflections
    const pm=new THREE.PMREMGenerator(renderer), es=new THREE.Scene();
    es.add(new THREE.Mesh(new THREE.BoxGeometry(30,30,30),new THREE.MeshBasicMaterial({color:0x202428,side:THREE.BackSide})));
    const sb=(w,h,x,y,z,k)=>{ const m=new THREE.Mesh(new THREE.PlaneGeometry(w,h),new THREE.MeshBasicMaterial({color:new THREE.Color(k,k,k),side:THREE.DoubleSide})); m.position.set(x,y,z); m.lookAt(0,0,0); es.add(m); };
    sb(14,10,0,14,0,3.2); sb(8,10,14,4,-6,1.6); sb(8,10,-14,4,6,1.1); sb(20,3,0,2,-14,.8);
    env=pm.fromScene(es,.03).texture; scene.environment=env;
  }catch(err){}
  grid=new THREE.GridHelper(40,40,0x000000,0x000000); grid.material.transparent=true; grid.material.opacity=.07; scene.add(grid);
  applyTheme();
}
let grid;
function applyTheme(){ const bg=cssVar('--scene')||'#12161b'; scene.background=lin(bg);
  const dark=new THREE.Color(bg).getHSL({h:0,s:0,l:0}).l<.5; grid.material.color.set(dark?0xffffff:0x000000); grid.material.opacity=dark?.05:.08;
  if(EN) EN.setColors(readColors()); }
...
function resize(){ const w=stage.clientWidth||1, h=stage.clientHeight||1; renderer.setSize(w,h,false); camera.aspect=w/h; camera.updateProjectionMatrix(); drawTimelineBase(); }
function defaultView(){ const e=EN.e; const len=e.zmax-e.zmin;
  return {target:V3(0,e.isFlat?.4:1.35,0),rad:8+.55*len+(e.isV?1.4:0)+(e.isFlat?1.6:0),az:135*DEG,pol:(e.isFlat?58:64)*DEG}; }
...
function updateCamera(dt){
  ...
  camera.lookAt(t);
  if(grid) grid.position.y=(EN?EN.e.cc.bot-1.05:-1.5)-(S.explode?2.6*smooth(S.explodeT):0);
}
...
const TOUR=[
 ...
 {t:'Look inside',x:'Cutaway slices the housings along the cylinder. The rust-coloured surfaces are the inside walls of the cut castings. Now the piston, rod and crankshaft are visible moving together.',a(){ setExplode(false); setView('cutaway'); select(null); resetView(false); }},
