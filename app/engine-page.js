...
function aboutLayout(){const a=ARCHS[S.arch];openDialog(a.full,`<p>${esc(a.tagline)}</p><dl class="layout-facts">${[['Layout',a.layout],['Firing order',a.firing.join('–')],['Firing interval',a.interval],['Balance',a.balance],['Packaging',a.packaging],['Character',a.character],['Common uses',a.uses]].map(([k,v])=>`<dt>${k}</dt><dd>${esc(v)}</dd>`).join('')}</dl>${a.note?`<p>${esc(a.note)}</p>`:''}<h3>Why this design exists</h3>${a.why.map(([q,t])=>`<h4>${esc(q)}</h4><p>${esc(t)}</p>`).join('')}<h3>Advantages</h3><ul>${a.pros.map(t=>`<li>${esc(t)}</li>`).join('')}</ul><h3>Trade-offs</h3><ul>${a.cons.map(t=>`<li>${esc(t)}</li>`).join('')}</ul><h3>Examples</h3>${a.examples.map(x=>`<h4>${esc(x.e)}</h4><p>${esc(x.spec)} · ${esc(x.cars)}. ${esc(x.why)}</p>`).join('')}<p class="small">Representative configurations with fixed bore, stroke and timing. Illustrative pressure and gas torque exclude inertia and friction. Real firing orders and numbering vary. Principles: Heywood, Internal Combustion Engine Fundamentals; Bosch Automotive Handbook.</p>`);}
...
$('#sceneReset').onclick=()=>resetEngineScene();$('#sceneAbout').onclick=()=>aboutLayout();
...
for(const id of ['engineView','lab','discover']){$('#'+id).setAttribute('role','tabpanel');$('#'+id).setAttribute('aria-labelledby','tab-'+(id==='engineView'?'engine':id));}
function restoreHash(){try{if(location.hash.startsWith('#scene=')){const s=validScene(JSON.parse(decodeURIComponent(location.hash.slice(7))));if(s)restoreScene(s);}}catch{/* malformed links are ignored */}}
